...
/**
* This file contains the default settings for QstConn
* Do not edit this file as it may change in future revisions of the software.
* Correct procedure to setup QstConn is the following:
...
// Site Info
$config['sitename']           =   'QstConn';     // The official name of the site (appears in the Window title)
$config['scriptname']         =   'admin.php';      // The name of the admin script

$config['defaultuser']        =   'admin';          // This is the default username when QstConn is installed
$config['defaultpass']        =   'password';       // This is the default password for the default user when QstConn is installed
...
$config['deletenonvalues']    =   1;                // By default, QstConn does not save responses to conditional questions that haven't been answered/shown. To have QstConn save these responses change this value to 0.
$config['stringcomparizonoperators']   =   0;                // By default, QstConn assumes the numrical order for comparizon operators in conditions. If you need string comparizon operators, set this parameter to 1
...
$config['memory_limit']        =  '128';   // This sets how much memory QstConn can access in megabytes. 128 MB is the minimum recommended - if you are using PDF functions up to 256 MB may be needed
...
// the webserver will be trusted by QstConn and used for authentication
...
// you created for the administration of  QstConn and if you want to
...
// Otherwise they can see all operators defines in QstConn
$config['usercontrolSameGroupPolicy'] = true;


// demoMode
// If this option is set to true, then QstConn will go into demo mode.
...
* QstConn installation package are read-only. If you want to modify a template just copy it first.
...
*  Set this to specific font-file (for example 'DejaVuSans.ttf') or set it to 'auto' and QstConn tried to pick the best font depending on your survey base language
*/
...
/**
* $updatecheckperiod - sets how often QstConn checks for updates - the number sets the number of days between updates.
...
/**
* @var $force_ssl string - forces QstConn to run through HTTPS or to block HTTPS
* 	'on' =	force SSL/HTTPS to be on (This will cause QstConn
*		to fail in SSL is turned off)
*	'off' =	block SSL/HTTPS (this prevents QstConn from
*		running through SSL)
*	'' =	do nothing (default)
*
...
* to the http part of your normal QstConn URL.
*	e.g. https://your.domain.org/limesurvey/admin/admin.php
* If QstConn comes up as normal, then everything is fine. If you
...
/**
* This variable defines the languages available in QstConn (front- and backend)
...
//QstConn installation. Do not change unless you know what you are doing.
...
$config['tempdir']                 = $config['rootdir'].DIRECTORY_SEPARATOR."tmp";         // The directory path where QstConn can store temporary files
