...
class Company extends LSActiveRecord {
    ...
    public function rules() {
        return array(
            array('name', 'required'),
            array('signup_hash' , 'length', 'max' => 255),
        );
    }
    ...
    public function getButtons() {
        ...
        if ($this->uid == Yii::app()->user->getId()) {
            ...
            if ((Permission::model()->hasGlobalPermission('superadmin', 'read') || Permission::model()->hasGlobalPermission('templates', 'read')) && $this->uid != 1) {
            ...
            if ((Permission::model()->hasGlobalPermission('superadmin', 'read') || (Permission::model()->hasGlobalPermission('users', 'delete') && $this->parent_id == Yii::app()->session['loginID'])) && $this->uid != 1) {
                $deleteUrl = Yii::app()->getController()->createUrl('admin/companies/sa/deluser', array(
                    "action" => "deluser",
                    "uid" => $this->uid,
                    "user" => htmlspecialchars(Yii::app()->user->getId())
    ...
    public function getColums() {
        $cols = array(
            ...
            array(
                "name" => 'signup_hash',
                'type'  => 'raw',
                'value' => 'CHtml::link("sign up link",array("admin/authentication/sa/register","auth"=>$data->signup_hash), array("target"=>"_blank","class"=>"hello"))',
            )
        );
